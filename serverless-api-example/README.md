...
```bash
serverless-api-example$ sam build
```
...
```bash
serverless-api-example$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
serverless-api-example$ sam local start-api
serverless-api-example$ curl http://localhost:3000/
```
...
```bash
serverless-api-example$ sam build
serverless-api-example$ sam deploy
```
...
```bash
serverless-api-example$ sam logs -n NetCodeWebAPIServerless --stack-name serverless-api-example --tail
```
...
```bash
serverless-api-example$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name serverless-api-example
```
